...
一个典型的逐点连续但不一致连续的场景是 $f(x) = \frac{1}{x}$ 在区间 $(0, \infty)$ 上：对于每一个点 $a \in \frac{1}{x}$ ，$\exists$ 方都能给出合适的 $\delta$ 来证明连续，但如果要求 $\exists$ 方先选出 $\delta$，则 $\forall$ 方可以针对这个 $\delta$ 构造一个非常接近 $0$ 的 $a$ 来使命题不成立。
